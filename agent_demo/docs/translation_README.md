...
- AI 生成中文回复，或客服写中文私有备注草稿时，系统可以自动翻译成用户语言，再发送给用户。
...
如果开启了出站翻译，AI 的中文回复会自动翻译成用户语言。

客服需要用中文回复外语用户时，请把中文写成 **私有备注**，不要直接发送公开回复。系统会把这条中文私有备注翻译成用户语言，并把翻译结果作为公开消息发给用户。
...
后续同一个会话里，如果客服写中文私有备注草稿，系统就知道应该翻译成哪种语言。
...
如果公开回复无法安全翻译，系统会按 `PUBLIC_REPLY_FALLBACK_LANGUAGE` 选择兜底语言，选项为 `vi` 或 `en`。`.env` 里只写语言代码，不写整句兜底文案；实际兜底语句维护在 `src/customer_agent/support_templates.py` 的 `PUBLIC_REPLY_FALLBACKS`。
...
- 客服写的是不是私有备注；公开回复已经对用户可见，系统不会再二次翻译。
- 系统是否已经识别过该会话用户语言。
- 服务是否刚重启导致语言缓存丢失。
